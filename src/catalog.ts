...
export class Catalog {
  ...
  /**
   * Per-(site, ability) schema cache. Ability schemas don't change between
   * plugin deploys, so re-fetching on every wp_get_ability call is pure
   * waste. Same TTL as the site catalog — refreshing one refreshes the other
   * naturally, and stale schemas would surface via a downstream execute error
   * anyway.
   */
  private infoCache = new Map<string, AbilityInfoEntry>();
  ...
  async getAbilityInfo(siteId: string, abilityName: string, force = false): Promise<any> {
    const key = `${siteId}\u0000${abilityName}`;
    const cached = this.infoCache.get(key);
    if (!force && cached && Date.now() - cached.fetchedAt < this.config.catalogTtlMs) {
      return cached.info;
    }
    const result = await this.client(siteId).callTool(INFO_TOOL, { ability_name: abilityName });
    const info = extractToolPayload(result);
    this.infoCache.set(key, { info, fetchedAt: Date.now() });
    return info;
  }

  /**
   * List distinct namespace groups on a site (e.g. "popup-maker", "fluent-crm").
   * Cheaper for the LLM than the full ability list — used by wp_list_sites
   * for the "what's roughly on each site" view before drilling in.
   */
  async siteGroups(siteId: string): Promise<{ groups: string[]; error?: string; abilityCount: number }> {
    const cat = await this.getCatalog(siteId);
    if (cat.error) return { groups: [], error: cat.error, abilityCount: 0 };
    const groups = [...new Set(cat.abilities.map((a) => a.group).filter((g): g is string => !!g))].sort();
    return { groups, abilityCount: cat.abilities.length };
  }
